...
/* Function start: 0x406FC0 */ /* DEMO ONLY - no full game match */
...
/* Function start: 0x407070 */ /* DEMO ONLY - no full game match */
SCI_Dialog::~SCI_Dialog() {
    MMPlayer* mc;
    MMPlayer2* dlg;
    Sprite* spr;
...
/* Function start: 0x407240 */ /* DEMO ONLY - no full game match */
...
/* Function start: 0x407380 */ /* DEMO ONLY - no full game match */
...
/* Function start: 0x4074E0 */ /* DEMO ONLY - no full game match */
...
/* Function start: 0x407650 */ /* DEMO ONLY - no full game match */
int SCI_Dialog::Exit(SC_Message* msg) {
    ...
    case 0x13:
        ...
        if (dq == 0) {
            dq = new SC_Question(msg->sourceAddress, this);
            if (dq->state == 2) {
                delete dq;
            } else {
                queue = field_610;
    ...
    case 0x14:
        dq = FindDialogById(msg->sourceAddress);
        g_FlagManager_0046a6e8->SetFlag(msg->sourceAddress, 1);
        if (dq != 0) delete dq;
        break;
...
/* Function start: 0x407A40 */ /* DEMO ONLY - no full game match */
...
int SCI_Dialog::LBLParse(char* line) {
    ...
    if (strcmp(token, "BACKGROUND") == 0) {
        ...
        dq = new SC_Question(questionId, this);
        if (dq->state == 2) {
            delete dq;
        } else {
            queue = field_610;
