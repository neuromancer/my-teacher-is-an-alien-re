...
$(OUT_DIR)/%.obj: src/%.cpp
	@mkdir -p $(OUT_DIR)
	@wine cmd /c "compilers\\msvc420\\bin\\VCVARS32.BAT x86 > out\\useless && $(CC) $(CFLAGS) $< /Fo$@" 2>/dev/null

# Rule to compile .cpp files to .asm files
$(OUT_DIR)/%.asm: src/%.cpp
	@mkdir -p $(OUT_DIR)
	@wine cmd /c "compilers\\msvc420\\bin\\VCVARS32.BAT x86 > out\\useless && $(CC) $(CFLAGS) $< /Fa$@ /Foout\\out.obj"   2>/dev/null
