        ...
        if label and label not in jump_table_targets:
            ...
            # Large blocks with $T refs are main code using compiler temporaries.
                ...
                if len(block_instructions) <= 8 or '??3@' in block_text:
                    is_seh_funclet = True
            # Check for destructor cleanup funclets: small blocks that jump to
            # a C++ destructor (??1) or operator delete (??3).
            # These are generated by the compiler for exception cleanup.
            elif '??1' in block_text or '??3' in block_text:
                block_instructions = [l for l in block_stripped if l and not l.endswith(':')]
                if len(block_instructions) <= 4:
                    is_seh_funclet = True
