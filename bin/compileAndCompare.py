...
def _filter_seh_funclets(assembly, seh_code, jump_table_targets):
    ...
    for i, line in enumerate(stripped):
        # Match any label: $L12345, $call_insert$12345, $send_remove_msg$12345, etc.
    ...
    for start, end, label in blocks:
        ...
        if label and re.match(r'^\$L\d+$', label) and label not in jump_table_targets:
            # SEH funclets are exception cleanup handlers that end with 'ret 0'
            # (returning from the exception handler). Real code blocks end with
            # jmp/je/jne etc. to other labels.
            block_instructions = [l for l in block_stripped
                                  if l and not l.endswith(':')
                                  and not (l.startswith('?') and '@@' in l)]
            last_instr = block_instructions[-1].split(';')[0].strip() if block_instructions else ''
            ends_with_ret = last_instr.startswith('ret')
            ...
            # Must end with ret to be a true SEH funclet.
            elif re.search(r'\$T\d+', block_text) and ends_with_ret:
                if len(block_instructions) <= 8 or '??3@' in block_text:
                    is_seh_funclet = True
            # Check for destructor cleanup funclets: very small blocks (1-2 instr)
            # that jump to a C++ destructor (??1) or operator delete (??3).
            # These are generated by the compiler for exception cleanup.
            # Larger blocks (3+ instr) may be real code called from normal flow.
            # Use instruction text only (not function name markers like ??1ClassName@@)
            elif True:
                instr_text = " ".join(block_instructions)
                if '??1' in instr_text or '??3' in instr_text:
...
def main():
    ...
    parser.add_argument("--demo", action="store_true", help="Use demo build (src-demo/out-demo)")
    args = parser.parse_args()

    out_dir = "out-demo" if args.demo else "out"
